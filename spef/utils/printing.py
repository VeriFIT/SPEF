...
def print_help(screen, win, env, exit_mess, title, actions):
    ...
    if title:
        ...
        screen.addstr(
            line, int(max_cols / 2 - len(title) / 2) + 1, title, curses.A_NORMAL
        )
        line += 1
...
def parse_line_into_sublines(line, max_cols):
    ...
    while words:
        ...
        while len(res_line) + len(word) < max_cols:
...
def show_user_input(
    screen, user_input, max_rows, max_cols, env, color=None, title=None
):
    screen.erase()
    screen.border(0)
    ...
    if user_input:
        ...
        # split string into list of words and spaces
        words = re.split(r"(\S+)", user_input_str)
        split_words = []
        for word in words:
        ...
        while words:
            ...
            # check if the word fits in the line
            while len(line) + len(word) < max_cols:
                """add word to the line"""
                line += word
                del words[0]

                """ get cursor position """
                # all characters (from begining to current pointer) were processed
                if cnt_total == user_input.pointer:
                    # save current cursor position: row + counter of characters on this row (as column)
                    current_cursor = (
                        row,
                        cnt_on_line,
                    )
                # process new added word in line, by its symbols (characters)
                for _ in word:
                    cnt_total += 1
                    cnt_on_line += 1
                    # with each character check if all were processed
                    if cnt_total == user_input.pointer:
                        current_cursor = row, cnt_on_line
                if not words:
                    break
        ...
        if cwd.is_empty():
            if env.filter_not_empty():
                screen.addstr(
                    1,
                    1,
                    "* No file matches with current filter *",
                    curses.A_NORMAL | curses.A_BOLD,
                )
            else:
                screen.addstr(
                    1, 1, "* This directory is empty *", curses.A_NORMAL | curses.A_BOLD
                )
        else:
            i = 1
            for dir_name in dirs:
                ...
                if env.show_solution_info and cwd.dirs_info:
                    ...
                    if infos:
                        space = 2  # visual space between dir name and its info
                        stop = len(txt) + space
                        x = max_cols
                        for item in infos:
                            info, col = item
                        ...
                        if x > stop:  # empty space between dir name and its info
                            screen.addstr(
                                i, len(txt) + 1, " " * (x - stop - 1 + space), coloring
                            )
...
def rewrite_one_line_in_file(env, line_num):
    ...
    # shift for line numbers
    shift = len(env.line_numbers) + 1 if env.line_numbers else 1
    ...
    if buffer is not None:
        ...
        if tokens is not None:
            ...
            for token in tokens:
                ...
                if (y - 1 + win.begin_y == win.cursor.row - win.row_shift) and (
                    win.col_shift > 0
                ):
                    old_position = position
                    # position simulates token printing
                    position += len(text[: max_cols - position - 1])
                    if position < win.col_shift:
                        continue
                    if old_position <= win.col_shift:
                        # token text is between pages (before and after col shift position)
                ...
                if text == "\n":
                    ...
                    if x + len(text) > max_cols + shift:
                        screen.addstr(
                            y, x, str(text[: max_cols + shift - x]), text_color
                        )
                        break
                    else:
            ...
            if (line_num - 1 + win.begin_y == win.cursor.row - win.row_shift) and (
                win.col_shift > 0
            ):
        ...
        if buffer:
            if tokens is not None:
                ...
                for token in tokens:
                    ...
                    if (
                        y > max_rows
                        or (y == max_rows and env.content_filter_on())
                        or y + win.row_shift > len(buffer)
                    ):
                        break
                    if x == 0:
                        x = 1 + shift
                    ...
                    """ column shift correction """
                    # y-1 bcs we start at line 1 not 0
                    if (y - 1 + win.begin_y == win.cursor.row - win.row_shift) and (
                        win.col_shift > 0
                    ):
                        old_position = position
                ...
                for row, line in enumerate(
                    buffer[win.row_shift : max_rows + win.row_shift]
                ):
                    ...
                    if (row + win.begin_y == win.cursor.row - win.row_shift) and (
                        win.col_shift > 0
                    ):
                        line = line[win.col_shift + 1 :]
                    if len(line) > max_cols - 1:
                    ...
                    if env.line_numbers:  # row+1 bcs row starts from 0
                        screen.addstr(
                            row + 1, 1, str(row + 1 + win.row_shift), line_num_color
                        )
                    else:
    ...
    try:
        ...
        if report.data:
            for row, note in enumerate(
                report.data[win.row_shift : max_rows + win.row_shift]
            ):
                if row > max_rows:
                    break
...
def show_menu(
    screen, win, menu_options, env, keys=None, selected=None, color=None, title=None
):
    screen.erase()
    screen.border(0)
    ...
    try:
        ...
        if len(menu_options) > 0:
            ...
            screen.addstr(
                row + 1,
                1,
                "* There is no option to select from menu *",
                curses.A_NORMAL | curses.A_BOLD,
            )
