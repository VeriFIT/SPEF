...
def resize_all(stdscr, env, force_resize=False):
    ...
    try:
        if curses.is_term_resized(curses.LINES, curses.COLS) or force_resize:
            ...
            bu_win_new_row = max(
                min(bu_win_old_row, new_env.windows.brows_up.end_y - 2), 0
            )
            ...
            t_end_new_win = (
                new_env.windows.tag.end_y
                - new_env.windows.tag.begin_y
                - new_env.windows.tag.border
            )
            t_win_new_row = max(min(t_win_old_row, t_end_new_win - 2), 0)
            t_win_new_col = max(min(t_win_old_col, new_env.windows.tag.end_x), 0)
            c_end_new_win = (
                new_env.windows.center.end_y
                - new_env.windows.center.begin_y
                - new_env.windows.center.border
            )
            ...
            new_env.windows.brows.set_cursor(
                b_win_new_row + b_win_old_shift, b_win_new_col
            )
            new_env.windows.brows_up.set_cursor(
                bu_win_new_row + bu_win_old_shift, bu_win_new_col
            )
            new_env.windows.notes.set_cursor(
                n_win_new_row + n_win_old_shift, n_win_new_col
            )
            new_env.windows.tag.set_cursor(
                t_win_new_row + t_win_old_shift, t_win_new_col
            )
            ...
            vu_win_height = (
                new_env.windows.view_up.end_y - new_env.windows.view_up.begin_y - 1
            )

            v_win_new_shift = new_vertical_shift(
                v_win_old_shift, v_win_old_height, v_win_old_row, v_win_height
            )
            vu_win_new_shift = new_vertical_shift(
                vu_win_old_shift, vu_win_old_height, vu_win_old_row, vu_win_height
            )
...
def create_screens_and_windows(height, width, line_numbers=None):
    ...
    # +1 stands for a space between line number and text
    shift = 1 if line_numbers is None else len(line_numbers) + 1
    view_vin = Window(
        r_win_h,
        max(r_win_w - shift, 0),
        r_win_y,
