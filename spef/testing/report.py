...
def get_data_for_report(env, solution):
    ...
    try:
        ...
        log(f"get data for report | get solution data | {err} | {traceback.format_exc()}")
    ...
    try:
        ...
        if t_tags is not None and len(t_tags)>0:
            ...
            for test_name in test_names:
                ...
                if test_scoring_args is not None:
                    ...
                    description = test_scoring_args[1] if len(test_scoring_args)>1 else "-"
        ...
        log(f"get data for report | get test results | {err} | {traceback.format_exc()}")
...
def generate_report_from_template(env, solution):
    ...
    if data:
        ...
        jinja_env = Environment(loader = FileSystemLoader(report_dir), trim_blocks=True, lstrip_blocks=True)
